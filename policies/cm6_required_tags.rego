...
deny contains msg if {
	...
	count(missing) > 0
	quoted := [sprintf("\"%s\"", [m]) | some m in sort_array(missing)]
	msg := sprintf(
		"[CM-6] %s: missing required labels [%s]. Remediation: add the missing labels to the resource.",
		[resource.address, concat(", ", quoted)],
	)
}
